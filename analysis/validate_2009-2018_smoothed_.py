...
SIM_PATH  = os.path.join(DATA_DIR, '2009-2018-table.csv')
REAL_PATH = os.path.join(EMP_DIR, 'real-data2.csv')
EXP_NAME  = '2009-2018'
START_YEAR = 2009
...
# 平滑设置:对季度高频变量(GDP, Inflation, Unemployment)做4Q MA
SMOOTH_WINDOW = 4

# 消费税上调时点: 2014年4月 (8%消费税,Abe政府第一次上调)
TAX_HIKE_QUARTER = '2014Q2'
TAX_HIKE_LABEL   = 'Tax hike\n5% → 8%'
...
# Pension 对齐: no lag, 2010-2017 (drop 2009 and 2018)
...
n_pen     = 8
pen_real_years = list(range(START_YEAR + 1, START_YEAR + 9))  # 2010-2017
...
print(f'Pension: no lag, {pen_real_years[0]}-{pen_real_years[-1]}, N={n_pen} points')
print(f'Smoothing: 4Q MA applied to GDP, Inflation & Unemployment (per-run, then aggregated)')

# ============================================================
# 波动率对比(平滑前后 / sim vs real)
# ============================================================
print('\n--- Volatility comparison (σ) ---')
print(f'{"Variable":<15}{"Real raw":>12}{"Sim raw":>12}{"Real 4QMA":>12}{"Sim 4QMA":>12}{"Sim/Real raw":>14}{"Sim/Real MA":>14}')
for name, rraw, sraw, rsm, ssm in [
    ...
    ('Unemployment', q_real_unemp, q_sim_unemp, q_real_unemp_sm, q_sim_unemp_sm),
]:
    rraw_sd = np.std(rraw); sraw_sd = np.std(sraw)
    rsm_sd  = np.std(rsm);  ssm_sd  = np.std(ssm)
    ratio_raw = sraw_sd/rraw_sd if rraw_sd > 0 else np.nan
    ratio_ma  = ssm_sd/rsm_sd if rsm_sd > 0 else np.nan
    print(f'{name:<15}{rraw_sd:>12.5f}{sraw_sd:>12.5f}{rsm_sd:>12.5f}{ssm_sd:>12.5f}'
          f'{ratio_raw:>14.2f}{ratio_ma:>14.2f}')
...
# ============================================================
# 图2 (主图): 模拟vs真实 四联图 — GDP/Inflation/Unemployment 用 4Q MA
# ============================================================
print('\n--- 图2: 模拟 vs 真实 四联图 (季度变量: 4Q MA) ---')
fig2, axs2 = plt.subplots(2, 2, figsize=(14, 10))
fig2.suptitle(
    f'Simulated vs. Real — Japan {START_YEAR}–{START_YEAR+9}'
    f'\n(burn-in = {bi} ticks; GDP, Inflation & Unemployment: 4-quarter moving average; pension: no lag, {pen_real_years[0]}–{pen_real_years[-1]})',
...
# Inflation (4Q MA) - 加上 2014Q2 消费税上调标注
ax = axs2[0, 1]
...
# 标注: 2014Q2 消费税从5%上调到8% (Abe政府,2014年4月)
infl_ylim = (
...
try:
    tax_idx = q_labels.index(TAX_HIKE_QUARTER)
    ax.axvline(tax_idx, color='#555555', lw=0.8, ls='--', alpha=0.7, zorder=1)
    ax.text(tax_idx + 0.3, infl_ylim[1] * 0.78,
            TAX_HIKE_LABEL,
...
fig2.tight_layout()
fig2.savefig(f'{OUT_DIR}/fig09_outofsample_fit_2009-2018.png', dpi=600, bbox_inches='tight')
print('✓ 图2 (主图, 4Q MA)')
...
fig2r.suptitle(
    ...
    f'\n(burn-in = {bi} ticks; pension: no lag, {pen_real_years[0]}–{pen_real_years[-1]}) — Appendix / Robustness',
    fontsize=11, fontweight='bold', y=1.02)
...
ax.set_ylabel('Rate'); ax.set_title('Total Unemployment Rate (raw)', fontweight='bold')
ax.legend(fontsize=8, framealpha=0.9, edgecolor='#CCC')
ax.grid(True, alpha=0.2, ls='--')
ax.set_xticks(tick_pos)
ax.set_xticklabels([q_labels[i] for i in tick_pos], rotation=45, ha='right', fontsize=8)
...
fig2r.tight_layout()
fig2r.savefig(f'{OUT_DIR}/si_fig09_outofsample_fit_raw.png', dpi=600, bbox_inches='tight')
print('✓ 图2附录 (raw, 备查)')
...
fin_cfg = [
    ...
    ('lev_m',  'lev_s',  'Median Leverage (Debt / Equity)'),
]
...
fig3.tight_layout()
fig3.savefig(f'{OUT_DIR}/si_financial_2009-2018.png', dpi=600, bbox_inches='tight')
print('✓ 图3')

# ============================================================
# 4. 拟合统计表 (raw + 4Q MA 对比, 含 p 值)
# ============================================================
print('\n--- 拟合统计 ---')
...
fit_table.to_csv(f'{OUT_DIR}/tab12_fit_stats_2009-2018.csv', index=False, encoding='utf-8-sig')
print('\nFit statistics:')
